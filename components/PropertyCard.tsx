...
const PropertyCard: React.FC<Props> = ({ items, isFavorite, isRejected, toggleFavorite, toggleRejected, onClick }) => {
  ...
  // Calculate total if group, otherwise just main price
  const displayPrice = isGroup 
    ? items.reduce((s, i) => s + i.precioBaseNumerico, 0)
    : main.precioBaseNumerico;
  ...
  return (
    ...
    >
       ...
       <div className="bg-slate-50/80 p-3 flex justify-between items-start border-b border-slate-100">
          <div className="flex-1 min-w-0 pr-2">
             <div className="flex items-center gap-1 text-slate-500 mb-1">
                ...
                <span className="text-xs font-bold uppercase tracking-wide truncate">
                  {main.provincia || 'Ubicación ND'}, {main.canton}
                </span>
             </div>
             <div className="flex items-center gap-1.5 flex-wrap">
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${isVehicle ? 'bg-orange-50 text-orange-700 border-orange-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>
                   {isGroup ? `LOTE DE ${items.length}` : main.tipoBien.toUpperCase()}
                </span>
                {isCondo && (
          ...
          <button onClick={(e) => { e.stopPropagation(); toggleFavorite(main.id); }} className={`p-1.5 rounded-full shadow-sm border ${isFavorite ? 'bg-red-50 border-red-100 text-red-500' : 'bg-white border-slate-100 text-slate-300 hover:text-red-400'}`}>
