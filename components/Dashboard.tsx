...
import { formatCurrency, formatDate, convertToCRC } from '../utils/helpers';
import { CalendarClock, Car, Home, BarChart3, ChevronRight } from 'lucide-react';
...
const Dashboard: React.FC<Props> = ({ properties, onFilterByProvince, onSelect }) => {
  ...
  return (
    <div className="space-y-6 mb-8 animate-in fade-in duration-500">
      ...
      {/* Geographic Bar Chart - Interactive */}
...
const VerticalList = ({ title, icon, items, onSelect, color, isDate }: any) => (
        ...
        >
          ...
          <div className="flex flex-col min-w-0 flex-1">
             <span className="text-[10px] text-slate-400 font-mono leading-tight truncate">
               {prop.provincia || 'S/Provincia'}, {prop.canton}
             </span>
             <span className="text-xs font-semibold text-slate-700 truncate">
               {prop.tipoBien === 'Vehículo' ? prop.descripcion : `Exp: ${prop.numeroExpediente}`}
             </span>
          </div>

          {/* Info Right (Price/Date) */}
          <div className="text-right shrink-0">
             ...
             {isDate && (
               <div className="flex flex-col items-end">
                  ...
                  {prop.medidasNumericas && (
                     <div className="text-[9px] text-slate-400">{prop.medidasNumericas.match(/[\d,.]+/)?.[0]} m²</div>
                  )}
               </div>
             )}
          </div>
